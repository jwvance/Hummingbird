...
/* `#END` */
    ...
    *******************************************************************************/
    CY_ISR(MIDI1_UART_RXISR)
    {
        ...
        readData = MIDI1_UART_RXSTATUS_REG;

        if((readData & (MIDI1_UART_RX_STS_BREAK | MIDI1_UART_RX_STS_PAR_ERROR |
        ...
        while((readData & MIDI1_UART_RX_STS_FIFO_NOTEMPTY) != 0u)
        {
            ...
            if( increment_pointer != 0u )
            {
                ...
                if(MIDI1_UART_rxBufferWrite >= MIDI1_UART_RXBUFFERSIZE)
                {
...
#endif /* End MIDI1_UART_RX_ENABLED && (MIDI1_UART_RXBUFFERSIZE > MIDI1_UART_FIFO_LENGTH) */
    ...
    *******************************************************************************/
    CY_ISR(MIDI1_UART_TXISR)
    {
        ...
        {
            /* Check pointer. */
            if(MIDI1_UART_txBufferRead >= MIDI1_UART_TXBUFFERSIZE)
            {
...
#endif /* End MIDI1_UART_TX_ENABLED && (MIDI1_UART_TXBUFFERSIZE > MIDI1_UART_FIFO_LENGTH) */
